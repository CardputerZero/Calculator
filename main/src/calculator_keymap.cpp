...
// Fallback mirrors the symbol layer in the device keymap. The runtime file is
// authoritative when it is present on CardputerZero.
...
bool load_tca8418_keymap(const char *path)
{
    ...
    char line[256];
    while (runtime_keymap.size() < kMaxRuntimeEntries &&
        ...
        const uint32_t character = keysym_to_character(symbol_name);
        if (character != 0) {
            runtime_keymap.emplace(keycode, character);
        }
    }
